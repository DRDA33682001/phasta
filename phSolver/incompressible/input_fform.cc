...
int input_fform(phSolver::Input& inp)
{
  ...
  try {
    ...
    hitforcing.PowerInput = inp.GetValue("Power-input to forcing");
    hitforcing.EnableForcing = 0;
    if ((string)inp.GetValue("Enable Forcing HIT") == "True") {
      hitforcing.EnableForcing = 1;
    }
